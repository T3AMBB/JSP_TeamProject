...
public class ReplyDAO {
	...
	final String sql_selectAll_Board="SELECT * FROM REPLY LEFT OUTER JOIN MEMBER ON REPLY.MID=MEMBER.MID WHERE REPLY.BID=? ORDER BY REPLY.RID DESC";
	...
	public ArrayList<ReplyVO> selectAll_Board(ReplyVO rvo){
		ArrayList<ReplyVO> datas=new ArrayList<ReplyVO>();
		conn=JDBCUtil.connect();
		try {
			pstmt=conn.prepareStatement(sql_selectAll_Board);
			pstmt.setInt(1, rvo.getBid());
			ResultSet rs=pstmt.executeQuery();
